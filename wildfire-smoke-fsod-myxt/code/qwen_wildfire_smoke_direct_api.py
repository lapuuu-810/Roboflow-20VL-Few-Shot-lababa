...
Use only one label: smoke.

Important visual rules:
- Smoke is a gray, white, brownish, or bluish translucent plume/cloud/column from a wildfire or forest fire.
- Box the full visible smoke plume, including faint transparent edges when clearly connected to the smoke.
- Use one tight box per separate smoke plume. If the smoke mass is continuous, use one box around the whole smoke region.
...
- Avoid large empty sky/background margins; the box should cover smoke as tightly as possible.

Return only JSON: {"detections":[{"bbox_2d":[x1,y1,x2,y2],"label":"smoke","confidence":0.90}]}.
Coordinates must be integers from 0 to 1000 over the full image.
""".strip()
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "qwen_smoke_direct_smalltest")
    ...
    metrics.update({"method": "qwen_direct_wildfire_smoke_prompt", "images": len(eval_image_ids), "total_test_images": len(image_by), "records": len(records), "image_ids": eval_image_ids, "predictions": len(preds), "params": {k: (str(v) if isinstance(v, Path) else v) for k, v in vars(args).items()}, "counts": dict(Counter(int(p["category_id"]) for p in preds))})
