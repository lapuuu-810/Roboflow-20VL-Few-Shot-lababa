...
COLOR_PROMPTS = {
    1: "red Coca-Cola soda bottle",
    2: "orange Fanta soda bottle",
    3: "green Sprite soda bottle",
}
...
CLASS_ALIASES = {
    ...
    "red soda bottle": 1,
    "red bottle": 1,
    "red coca-cola soda bottle": 1,
    "fanta": 2,
    "orange": 2,
    "orange soda bottle": 2,
    "orange bottle": 2,
    "orange fanta soda bottle": 2,
    "sprite": 3,
    "green": 3,
    "green soda bottle": 3,
    "green bottle": 3,
    "green sprite soda bottle": 3,
}
...
def make_prompt() -> str:
    labels = "\n".join(f"- {cid}: {name}" for cid, name in COLOR_PROMPTS.items())
    return f"""
Detect every visible soda bottle in the shelf image. The dataset classes are based on bottle color and brand:
{labels}

Important visual rules:
- Class 1 coca-cola: red label/cap area, red Coca-Cola style bottle.
- Class 2 fanta: orange label/cap area, orange Fanta style bottle.
- Class 3 sprite: green label/cap area, green Sprite style bottle.
- Boxes should tightly cover the whole visible bottle, not just the label.
- Include partially visible bottles if enough of the bottle body is visible.
- Ignore background shelves, price tags, cardboard, reflections, and non-bottle objects.
- If bottles are arranged in rows, return each individual bottle separately.

Return only JSON in this exact form:
{{"detections":[{{"bbox_2d":[x1,y1,x2,y2],"label":"coca-cola","confidence":0.90}}]}}
...
def visualize(preds, image_by, out_dir: Path, count: int):
    ...
    colors = {1: (220, 30, 30), 2: (250, 145, 20), 3: (40, 180, 80)}
    vis = out_dir / "vis"
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "doubao_soda_direct_smalltest")
    ...
    metrics.update({
        ...
        "method": "doubao_direct_color_soda_bottle_prompt",
        "class_prompts": COLOR_PROMPTS,
