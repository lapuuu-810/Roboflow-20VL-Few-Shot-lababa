...
SOURCE_ROOT = Path('/data/LPP/few_shot/sam3-main/best_sam3/orionproducts-vtl2z-fsod-puhv')
...
def stage0_knowledge(raw_dir, api_key, model, timeout, retries, resume):
    ...
    raw = call_api(None, prompt, api_key, model, timeout, retries)
...
def stage1_gemini_direct(img, image_id, raw_dir, api_key, model, timeout, retries, resume, coord_mode):
    ...
    for det in obj.get('detections', obj.get('items', [])):
        ...
        if not box:
            continue
        out.append({'image_id': image_id, 'bbox': box, 'score': float(det.get('confidence', det.get('score', 0.5)) or 0.5), 'source': 'gemini_direct'})
    return out
...
def stage4_review(img, preds, raw_dir, api_key, model, timeout, retries, resume, max_side, low, high):
    uncertain = [(i, p) for i, p in enumerate(preds) if p['category_id'] in CLASS_NAMES and low <= p['gemini_confidence'] < high]
    if not uncertain:
        return preds
...
def main():
    ...
    ap.add_argument('--sam3-jsonl', nargs='*', default=[str(p) for p in sorted(SOURCE_ROOT.glob('predictions_gpu*.jsonl'))])
    ap.add_argument('--out', default=str(ROOT / 'gemini_orion_sku_pipeline_test.json'))
    ap.add_argument('--raw-dir', default=str(ROOT / 'gemini_orion_sku_pipeline_raw'))
    ap.add_argument('--use-gemini-direct', action='store_true')
    ...
    args = ap.parse_args()
    if not args.api_key:
        raise SystemExit('set GEMINI_API_KEY/GOOGLE_API_KEY')
    ...
    for image_id in ids:
        ...
        if args.use_gemini_direct:
            direct_cands = stage1_gemini_direct(img, image_id, raw_dir, args.api_key, args.model, args.timeout, args.retries, args.resume, args.direct_coord_mode)
