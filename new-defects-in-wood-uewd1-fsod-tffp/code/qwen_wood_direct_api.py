...
DS = "new-defects-in-wood-uewd1-fsod-tffp"
...
CLASS_NAMES = {1: "Crack", 2: "Dead knot", 3: "Holes", 4: "Live knot", 5: "knot with crack"}
CLASS_ALIASES = {
    "crack": 1, "cracks": 1, "split": 1, "wood crack": 1, "checking": 1,
    "dead knot": 2, "dead knots": 2, "dark knot": 2, "black knot": 2, "dead-knot": 2,
    "holes": 3, "hole": 3, "void": 3, "small holes": 3, "pin holes": 3, "worm holes": 3,
    "live knot": 4, "live knots": 4, "sound knot": 4, "healthy knot": 4, "live-knot": 4,
    "knot with crack": 5, "cracked knot": 5, "knot crack": 5, "knot-with-crack": 5,
}
...
def make_prompt() -> str:
    return """
Detect every visible wood defect and return tight bounding boxes around each defect region.
Use only these labels:
- Crack: long thin dark split, checking line, fissure, or open crack in the wood.
- Dead knot: dark or black dead knot, usually circular or oval, without healthy wood grain texture.
- Holes: small dark round holes, worm holes, pin holes, or voids.
- Live knot: intact knot with visible wood grain or ring texture, generally lighter and healthier than a dead knot.
- knot with crack: a knot region that visibly contains, crosses, or connects to a crack.
Include partially visible defects. Do not detect normal wood grain, stain, shadow, board edge, background, or uncertain marks.
Return only JSON: {"detections":[{"bbox_2d":[x1,y1,x2,y2],"label":"Crack","confidence":0.90}]}.
...
def convert_records(records, image_by, args):
    ...
    for rec in records:
        ...
        for det in dets:
            ...
            if cid is None:
                ...
                continue
            bbox = norm1000_to_xywh(det.get("bbox_2d", det.get("bbox")), int(im["width"]), int(im["height"]))
            if not bbox:
    ...
    return nms(preds, args.nms_thr), audit
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "qwen_direct_bbox_qwen3vl_full")
    ...
    gt = load_json(GT)
    image_by = {int(im["id"]): im for im in gt["images"]}
    records = load_json(args.out_dir / "qwen_records.json") if args.skip_api else run_qwen(args)[0]
