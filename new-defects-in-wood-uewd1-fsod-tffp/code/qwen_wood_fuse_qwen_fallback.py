...
def main():
    ...
    for name, rows in sources.items():
        print(name, len(rows), single[name]["mAP"], single[name]["mAP50"], single[name]["mAP75"])

    results = []
    for s3vl in [0.8, 0.9, 1.0, 1.1]:
        for s36 in [0.0, 0.15, 0.25, 0.35, 0.55, 0.75]:
            for s35 in [0.0, 0.15, 0.25, 0.35, 0.55, 0.75]:
                for sfb in [0.0, 0.05, 0.10, 0.20, 0.35, 0.55]:
                    for nthr in [0.45, 0.55, 0.65, 0.75, 0.85]:
    ...
    for row in results[:15]:
        print("fuse_best", row[:4], row[4], row[5])
    ...
    for scale in [0.90, 0.94, 0.98, 1.0, 1.02, 1.04, 1.05, 1.06, 1.08, 1.10, 1.14]:
        ...
        print("scale", scale, rec["mAP"], rec["mAP50"], rec["mAP75"])
