...
CLASS_ALIASES = {
    "fish": 1, "small fish": 1, "reef fish": 1, "fishes": 1,
    "jellyfish": 2, "jelly fish": 2, "medusa": 2,
    "penguin": 3, "penguins": 3,
    "puffin": 4, "puffins": 4,
    "shark": 5, "sharks": 5,
    "starfish": 6, "sea star": 6, "seastar": 6,
    "stingray": 7, "ray": 7, "manta ray": 7, "sting ray": 7,
}
...
def make_prompt() -> str:
    return """
Detect every visible aquarium animal in the image and return tight bounding boxes.

Official classes:
1 fish: ordinary fish with fins/tail; include small or partially visible fish.
2 jellyfish: translucent bell-shaped jellyfish with tentacles; often pale/transparent.
3 penguin: black-and-white penguin bird in aquarium/zoo scene.
4 puffin: seabird with puffin shape/beak; not a penguin.
5 shark: large elongated shark body, pointed snout, dorsal fin, shark silhouette. Do not label ordinary fish as shark.
6 starfish: star-shaped sea star on tank floor/glass/rocks.
7 stingray: flat ray body with wing-like fins and long tail; different from shark/fish.

Important visual rules:
- Return one box per individual animal.
- Use tight boxes around the whole visible animal body; include fins/tail/tentacles when visible.
- For schools of fish, detect each visible individual separately when distinguishable.
- Include partially occluded animals if enough body is visible.
- Ignore rocks, coral, bubbles, reflections, aquarium glass glare, labels, people, and background objects.
- If uncertain between fish and shark, use shark only for clear shark silhouette; otherwise fish.
- If uncertain between fish and stingray, use stingray only for clear flat ray body with wing-like fins.

Return only JSON: {"detections":[{"bbox_2d":[x1,y1,x2,y2],"label":"fish","category_id":1,"confidence":0.90}]}
...
def visualize(preds, image_by, out_dir: Path, count: int):
    ...
    colors = {1: (40, 160, 240), 2: (220, 120, 255), 3: (255, 255, 255), 4: (255, 150, 0), 5: (220, 40, 40), 6: (255, 210, 0), 7: (40, 220, 140)}
    vis = out_dir / "vis"
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "doubao_aquarium_direct_smalltest")
    ...
    metrics.update({
        ...
        "method": "doubao_direct_aquarium_prompt",
