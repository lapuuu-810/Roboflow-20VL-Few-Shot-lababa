...
from qwen_dental_sam3_candidate_api import GT, evaluate, iou_box, load_json, load_sam3_candidates, nms, save_json
...
def snap(api_preds, cands, max_dist=38.0):
    by=defaultdict(list)
    for c in cands: by[c['image_id']].append(c)
    out=[]
    for p in api_preds:
        pc=center(p['bbox']); best=None; best_score=-1
        for c in by[p['image_id']]:
            cc=center(c['bbox'])
            dist=math.hypot(pc[0]-cc[0], pc[1]-cc[1])
            # Favor candidates near the Qwen box center and overlapping/adjacent.
            score=-dist + 18*iou_box(p['bbox'], c['bbox']) + 8*float(c.get('score',0.5))
            if dist <= max_dist and score > best_score:
                best_score=score; best=c
        if best:
            out.append({'image_id':p['image_id'],'category_id':p['category_id'],'bbox':[float(v) for v in best['bbox']], 'score':round(float(p.get('score',0.8))*0.85 + float(best.get('score',0.5))*0.15,6)})
    return out
...
def filt(preds,nms_thr,max_img,max_cat):
    ...
    for p in preds:
        x,y,w,h=p['bbox']; area=w*h; cid=p['category_id']
        if area<50 or area>7000: continue
    ...
    out=[]
    for iid,items in byi.items(): out.extend(sorted(items,key=lambda z:z['score'],reverse=True)[:max_img])
    return out


def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--api',required=True); ap.add_argument('--crop'); ap.add_argument('--image-ids',default='0,1'); ap.add_argument('--out',required=True); args=ap.parse_args()
    ids=[int(x) for x in args.image_ids.split(',') if x.strip()]
    gt=load_json(GT); root=Path(__file__).resolve().parent
    cands=load_sam3_candidates(gt,[Path(p) for p in sorted(glob.glob(str(root/'predictions_gpu*.jsonl')))],ids,40,10000,0.0,0.65,1000)
       ...
       for nms_thr in [0.25,0.35,0.45,0.55]:
        for max_img in [16,24,32,50,100]:
         for max_cat in [6,10,16,40]:
          ...
          idset=set(ids); sub=dict(gt); sub['images']=[im for im in gt['images'] if im['id'] in idset]; sub['annotations']=[a for a in gt['annotations'] if a['image_id'] in idset]
          ...
          if best is None or rec[:2]>best[:2]:
           best=rec; print('best',round(rec[0],4),round(rec[1],4),'n',len(preds),'params',rec[2:7])
