...
python gemini_dental_sam3_candidate_api.py \
  ...
  --score-scale 1.0 \
  --final-nms 0.45 \
  --out gemini_dental_sam3crop_all_top35_rawapi_v1.json \
  --raw-dir gemini_dental_sam3crop_all_top35_rawapi_v1_raw \
  --resume \
  --evaluate

python snap_fuse_gemini_sam3.py \
  --api gemini_dental_sam3crop_all_top35_rawapi_v1.json \
  --image-ids all \
  --out gemini_dental_snapfuse_all_v1.json
