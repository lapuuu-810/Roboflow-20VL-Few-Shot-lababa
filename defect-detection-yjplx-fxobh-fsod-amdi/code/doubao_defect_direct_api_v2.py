#!/usr/bin/env python3
"""Direct Doubao API inference v2 for defect-detection-yjplx-fxobh-fsod-amdi.
...
Example small-batch run:
  ARK_API_KEY='...' python doubao_defect_direct_api_v2.py \
    --image-ids 136,115,88,48,26,152 \
    --out doubao_defect_direct_api_v2_small.json \
    --raw-dir doubao_defect_direct_api_v2_small_raw \
...
def salvage_detections(text: str):
    detections = []
    pattern = re.compile(
        r'"category_id"\s*:\s*(\d+).*?"bbox"\s*:\s*\[([^\]]+)\].*?"confidence"\s*:\s*([0-9.]+)',
        flags=re.S,
    )
    for match in pattern.finditer(text):
        try:
            cid = int(match.group(1))
            box = [float(x) for x in re.findall(r'-?\d+(?:\.\d+)?', match.group(2))[:4]]
            conf = float(match.group(3))
        except Exception:
            continue
        if len(box) == 4:
            detections.append({'category_id': cid, 'bbox': box, 'confidence': conf})
    if detections:
        return {'detections': detections}
    if re.search(r'"detections"\s*:\s*\[\s*\]', text) or re.search(r'"positions"\s*:\s*\[\s*\]', text):
...
def parse_json(text: str):
    ...
    for cand in candidates:
        ...
        # Doubao sometimes emits bare quoted strings inside an object, e.g.
        # ,"image_width..._4000","image_height..._3000". Drop those members.
        ...
        for variant in variants:
            try:
                ...
                last = exc
    salvaged = salvage_detections(text)
...
def call_api(images, prompt, api_key, model, timeout, retries):
    ...
    for label, img in images:
        ...
        content.append({'type': 'input_image', 'image_url': image_url(img)})
    payload = {
        'model': model,
...
def load_or_parse_raw(raw_json: Path, raw_txt: Path):
    if raw_json.exists():
...
def run(args):
    ...
    for image_id in image_ids:
        ...
        if 'fishplate' in passes:
            ...
            raw_txt = raw_dir / f'{key}_raw.txt'
            obj = load_or_parse_raw(raw_json, raw_txt) if args.resume else None
            if obj is not None:
...
def main():
    ...
    parser.add_argument('--api-key', default=os.environ.get('ARK_API_KEY') or os.environ.get('QWEN_API_KEY'))
    ...
    parser.add_argument('--out', default=str(ROOT / 'doubao_defect_direct_api_v2.json'))
    parser.add_argument('--raw-dir', default=str(ROOT / 'doubao_defect_direct_api_v2_raw'))
