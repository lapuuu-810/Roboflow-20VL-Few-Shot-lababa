...
def make_prompt() -> str:
    return """
Detect every visible lacrosse participant and return tight bounding boxes around the visible person, not around the stick alone.
Use only these labels:
- Goalie: lacrosse goalie, usually in or near the goal crease, with goalie gear or goalie stick.
- Longpole: field player carrying a long defensive pole, visibly longer than a normal short stick.
- Referee: game official, usually wearing a black-white striped shirt or referee uniform.
- Shortstick: field player carrying a regular short lacrosse stick; use this for normal attackers or midfielders.
Include partially visible people. Do not detect the ball, goal, stick-only regions, helmets without a body, spectators, or background people.
Return only JSON: {"detections":[{"bbox_2d":[x1,y1,x2,y2],"label":"Shortstick","confidence":0.90}]}.
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "qwen_direct_bbox_qwen3vl_smalltest")
