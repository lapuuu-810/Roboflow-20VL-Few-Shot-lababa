...
CLASS_ALIASES = {
    "button": 1, "command button": 1, "submit button": 1,
    "check box": 2, "checkbox": 2, "empty checkbox": 2, "unchecked checkbox": 2,
    "checked radio button": 3, "selected radio button": 3, "filled radio button": 3, "radio selected": 3,
    "checked box": 4, "checked checkbox": 4, "selected checkbox": 4, "tick checkbox": 4,
    "dropdown box": 5, "dropdown": 5, "select box": 5, "combo box": 5, "closed dropdown": 5,
    "dropdown expand": 6, "expanded dropdown": 6, "dropdown list": 6, "open dropdown": 6, "popup list": 6,
    "icon": 7, "glyph": 7, "ui icon": 7, "chevron": 7, "arrow icon": 7,
    "radio button": 8, "unchecked radio button": 8, "empty radio button": 8,
    "scroll bar": 9, "scrollbar": 9, "scroll thumb": 9, "scroll track": 9,
    "text box": 10, "textbox": 10, "input field": 10, "search field": 10, "text input": 10,
}
...
def make_prompt() -> str:
    return """
Detect all visible UI elements in the screenshot image. Return tight boxes around each UI control.

Official classes:
1 Button: clickable command rectangle such as submit/login/next/save/clear.
2 Check box: small empty square form-control checkbox; not a square icon tile.
3 Checked Radio button: small circular radio option with inner dot/fill/selected mark.
4 Checked box: small square checkbox with check mark/tick/fill.
5 Dropdown box: full closed select/combo field rectangle with selected text/value and usually a down arrow.
6 Dropdown expand: expanded/open dropdown list or popup panel area, often a tall menu/list rectangle.
7 Icon: standalone UI symbol/glyph, including search/calendar/menu/user/star icons and small chevron/down-arrow icons inside dropdown fields. Mouse cursor/pointer arrow is not a target.
8 Radio button: small empty circular ring option, unselected radio control.
9 Scroll bar: scroll track/thumb on page or container edge.
10 Text box: editable input/search field rectangle, not a command button.

Critical rules:
- Empty circular ring => Radio button (8). Circular control with filled center => Checked Radio button (3).
- Empty small square form control => Check box (2). Checked/ticked square => Checked box (4).
- Big square tile containing a symbol is usually Icon (7), not checkbox.
- Closed dropdown/select field is Dropdown box (5). Open dropdown panel/list is Dropdown expand (6).
- Small arrow/chevron inside a dropdown may also be Icon (7) if individually visible.
- Use tight boxes around the visible control only, not nearby text labels unless the text is inside the control.
- Detect all visible target instances and avoid duplicate boxes for the same element.

Return only JSON: {"detections":[{"bbox_2d":[x1,y1,x2,y2],"label":"Radio button","category_id":8,"confidence":0.95}]}
    ...
    for rec in records:
        ...
        for det in dets:
            if not isinstance(det, dict):
                ...
                continue
            cid = int(det.get("category_id")) if str(det.get("category_id", "")).isdigit() and int(det.get("category_id")) in CLASS_NAMES else label_to_id(det.get("label", det.get("category_name", det.get("category"))))
            if cid is None:
...
def visualize(preds, image_by, out_dir: Path, count: int):
    ...
    colors = {1: (220, 60, 60), 2: (255, 170, 0), 3: (220, 0, 220), 4: (255, 90, 0), 5: (100, 80, 255), 6: (0, 80, 255), 7: (0, 200, 80), 8: (0, 180, 255), 9: (80, 80, 80), 10: (40, 140, 240)}
    vis = out_dir / "vis"
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=ROOT / "doubao_all_elements_direct_smalltest")
    ...
    metrics.update({
        ...
        "method": "doubao_direct_all_elements_ui_prompt",
